...
def evaluate_model(Method, phase):
    ...
    with open(f"input/data/{test_target_list}", 'r') as reader:
        target_list = json.load(reader)

    evaluation_dataset = load_from_file(f"input/data/{test_prompt_data}")
...
def get_score(Method, phase):
    ...
    trigger_groundtruth = json.load(open(os.path.join("input/data", test_trigger_data), 'r'))
